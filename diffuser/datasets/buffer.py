...
class ReplayBuffer:
    ...
    def add_path(self, path):
        ...
        # assert path_length <= 300

        if path['terminals'].any():
            assert (path['terminals'][-1] ==
                    True) and (not path['terminals'][:-1].any())
        ...
        if path['terminals'].any() and self.termination_penalty is not None:
            ...
            self._dict['rewards'][self._count,
                                  path_length - 1] += self.termination_penalty
    ...
    def finalize(self):
        ...
        self._add_attributes()
        print(
            f'[ datasets/buffer ] Finalized replay buffer | {self._count} episodes')
